...
exports.createProduct = async (req, res) => {
    try {
        ...
        const { title, description, currencyId, currencyFormat, isFreeShipping, style,availableSizes } = data
        ...
        if (style) {
            if (!validator.isValidString(style)) return res.status(400).send({ status: false, message: 'please provide style' })
        }
        ...
        return res.status(201).send({ status: true, message: "Product created successfully", data: createdProduct })
    }
    catch (err) {
        console.log(err)
...
//=================================================// GET PRODUCT //============================================================================//
 exports.getproducts=async function(req,res){
    ...
    if (size ) {
        if (!validator.isValidSize(size)) return res.status(400).send({ status: false, message: 'please provide valid Sizes' })
    ...
    console.log(finalFilters.price) /////
    if (priceSort) {
        if (!(priceSort == "1" || priceSort == "-1")) return res.status(400).send({status: false, message: "values can either be 1 or -1"
        })
