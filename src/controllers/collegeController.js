...
const collegeDetailsApi = async function (req, res) {
  ...
  try {
    ...
    if(!validKey[0]){return res.status(400).send({ status: false, message: "Put collegeName in the query" })}
    ...
    if (!collegeId) { return res.status(404).send({ status: false, message: "College does not found" }) }

    if (collegeId.isDeleted) { return res.status(400).send({ status: false, message: "This college does not provide internship" }) }
    ...
    if (internsNameWithCollege.length === 0) { return res.status(400).send({ status: false, message: "NO intern found from this college" }) }

    let newData = {
      name: collegeId.name,
      fullName: collegeId.fullName,
      logoLink: collegeId.logoLink,
      countInterns: internsNameWithCollege.length,
